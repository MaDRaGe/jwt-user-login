...
const tokens = async (userId) => {
  ...
  const refresh = uuid();
  await new Refresh({ refresh, userId }).save(
    (err) => err && console.log("Refresh: Error on save!")
  );
...
router.post("/signin", async (req, res) => {
  try {
    ...
    if (!bcrypt.compare(password, user.password))
      return hError(403, "Password is invalid");
...
router.post(
  "/signout",
  jwtMiddleware({ secret: config.jwtSecret }),
  async (req, res) => {
